...
export const metadata = { title: "รูปภาพ · ผู้ดูแล Terra Pride" };
...
export default async function AdminPhotosPage() {
  ...
  return (
    <div className="flex flex-col gap-8">
      <header>
        <h1 className="text-2xl font-semibold tracking-tight">รูปภาพทั้งหมด</h1>
        <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
          {photos.length} รูป · {activeCount} ใช้งาน · {deletedCount} ถูกลบ
        </p>
      </header>
      ...
      {photos.length === 0 ? (
        <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-zinc-300 bg-white px-6 py-16 text-center dark:border-zinc-700 dark:bg-zinc-900">
          ...
          <p className="text-xs text-zinc-400 dark:text-zinc-500">
            รูปจากผู้เข้าร่วมจะปรากฏที่นี่
          </p>
        </div>
      ) : (
            ...
            >
              <div className="relative aspect-[4/3] overflow-hidden bg-zinc-100 dark:bg-zinc-800">
                ...
                <img
                  ...
                  alt={photo.caption || "รูปที่อัปโหลด"}
                  loading="lazy"
              ...
              <div className="flex flex-1 flex-col gap-3 p-4">
                <div className="flex-1">
                  <p className="line-clamp-2 text-sm font-medium text-zinc-900 dark:text-zinc-100">
                    {photo.caption || (
                      <span className="italic text-zinc-400">ไม่มีคำบรรยาย</span>
                    )}
                  </p>
                  <p
                  ...
                  <p className="mt-0.5 text-xs text-zinc-400 dark:text-zinc-500">
                    {photo.isDeleted && photo.deletedAt
                      ? `ลบเมื่อ ${formatDate(photo.deletedAt)}`
                      : `อัปโหลด ${formatDate(photo.createdAt)}`}
                  </p>
                </div>
